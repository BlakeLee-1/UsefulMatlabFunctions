...
function [M,delT] = process(M, temp)
    ...
    BathTemp = [];
        ...
        xlabel('Field (T)')
        ylabel('\kappa (W/Km)')
        title('MgCr_{2}O_{4} Heat Capacity v Field')
end

%         M.delT = M.cutHotTemp - M.cutColdTemp;  %Convert to this run's Temp difference
%         M.powers = (M.cutHtrCurrent.*M.cutHtrCurrent).*M.HTRres;    %Square this run's currents
%         [M.fitdata, M.error] = polyfit(M.powers,M.delT, 1);
%         M.rsq = 1 - (M.error.normr/norm(M.delT - mean(M.delT)))^2;
%         M.kappa = M.fitdata(1);
%         M.intercept = M.fitdata(2);
%         kappas = [kappas, M.kappa];
%         BathTemp = [BathTemp, mean(M.cutBathTemp)];
% 
% 
% 
%     end
...
%    end
%     
%     plot(BathTemp, kappas/alpha, 'or', 'MarkerFaceColor', 'r')
