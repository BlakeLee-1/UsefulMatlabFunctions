function [f,M] = ConductivityDriver()
    ...
    %ProcessData(M);
    kappa(M0,'b', '0T')
    kappa(M18P,'g', '18T Perpendicular')
    %[f,~] = resistanceCalc(M);
...
function [M] = LoadData(temp) %Function to create a structure M
    ...
    filedata = dir(['*',temp,'*.mat']); %define structure filedata as all files containing string
...
function [M] = kappa(M, color, name)
    ...
    figure(3);
    plot(M(imax).Time,M(imax).HotTemp-M(imax).ColdTemp, 'b')
    figure(1);
    area = 5.25e-8;
    Length = 1.875e-4;
    alpha = Length/area;
    rsq
    kappapure = kappas;
   for i = length(kappas):-1:1
       if (BathTemp(i) > 12.7) && (BathTemp(i) < 13.14)
    ...
    ylabel('\kappa (W/Km)')
    title('MgCr_{2}O_{4} Thermal Conductivity v Temperature')
end
